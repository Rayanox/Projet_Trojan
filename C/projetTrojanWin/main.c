...
//envoyerEmail(ip);
	...
	int config, association, ecoute, socClient, fermeture, reception, envoie;
	...
	ecoute = listen(soc, 0); // a modifier si activer plusieurs connections à écouter en même temps
	if(ecoute != -1) printf("\n\nActivation de l'écoute réussie\n\n");
	else {printf("Probleme d'écoute");exit(0);}

	printf("\n\nEn attente de connexion ...\n\n");

	int taille_configClient = sizeof(configClient);
	socClient = accept(soc, (SOCKADDR *) &configClient, &taille_configClient);

	if(socClient != INVALID_SOCKET) printf("\n\nConnexion réussie\n\n");
	else printf("\n\nError\n\n");
...
reception = recv(socClient, buffer, sizeof(buffer), 0);
